...
test("handles all JSON response cases from backend", async () => {
    const form = document.getElementById("new-account-form");
    ...
    global.fetch.mockResolvedValueOnce({
        status: 400,
        json: async () => ({ error: "Generic error from backend" })
    });
    await submitAndWait();
    expect(errorEl.textContent).toBe("Generic error from backend");
